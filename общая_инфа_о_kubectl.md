...
### Проблема `Service` с запуском на ARM

Не могу запустить сервис как обычно показано в доке. Что я сделал, заэплаил файл `"./06_deployments/kuber_service_for_test.yaml"` с типом `NodePort`, возможно с другим типом будет лучше, и запустил minikube с другим названием через `--profile`. Запуская сервис, по идее можно к нему обратиться через ip миникуба и порт который повесил сервис.
Порт, который повесил сервис можно узнать через команду

`kubectl get service {{название-сервиса}} -o yaml`

и найти там ключ `nodePort`, его значение и есть прокинутый порт для нашего приложения.
Что-бы узнать ip миникуба

`minikube ip`

Но у меня не вышло. Кубер ругался что якобы нужно сначала запустить миникуб.🥲
[Вот тут](https://github.com/kubernetes/minikube/issues/13788) я узнал что на чипе M1/M2 запуская minikube через докер, нельзя к нему обратиться обычным способом через ip minikube. Там говорится что на чипе ARM от apple нужно запускать сервис выделяя ему терминал, через команду

`minikube service {{название-сервиса}}`

И так же, как в случае с ip, у меня появлялась ошибка с запуском сервиса, кубер ругался что minikube не запущен😁🤦‍♂️

Дальше разбирая проблему и до меня дошло, что что-бы узнать ip моего запущенного миникуба, мне нужно прописать команду не просто

`minikube ip`, а через команду `minikube ip --profile {{название-миникуба}}`

Таким образом я смог получить ip. И дальше таким же образом, я смог запустить сервис через команду

`minikube service {{название-сервиса}} --profile {{название-миникуба}}`
